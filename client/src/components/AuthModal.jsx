...
import { X, Shield, Lock, UserPlus, CheckCircle, Eye, EyeOff, FileSignature } from 'lucide-react';
...
export default function AuthModal({
  ...
  const [dataConsentSignature, setDataConsentSignature] = useState('');
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    const trimmedConsentSignature = dataConsentSignature.trim();
    ...
    if (!dataConsentAccepted) {
      setRegError('Please read and accept the Data Privacy Consent Form before registering.');
      return;
    }

    if (!trimmedConsentSignature) {
      setRegError('Please sign the Data Privacy Consent Form by typing your full name.');
      return;
    }

    if (trimmedConsentSignature.toLowerCase() !== trimmedName.toLowerCase()) {
      setRegError('The consent signature must match the full name entered above.');
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:5000/api/auth/signup', {
        ...
        body: JSON.stringify({
          ...
          dataConsentAccepted,
          dataConsentSignature: trimmedConsentSignature,
        }),
      });
      ...
      if (err.message && (err.message.includes('Failed to fetch') || err.message.includes('NetworkError'))) {
        ...
        const newUser = {
          ...
          dataConsentSignature: trimmedConsentSignature,
          dataConsentSignedAt: new Date().toISOString(),
          approved: true,
          history: [
            {
                ...
                <form onSubmit={handleRegisterSubmit} className="space-y-4">
                  ...
                  <div className="rounded-xl border border-emerald-300 bg-emerald-50/80 p-4 space-y-3">
                    <div className="flex items-start gap-3">
                      <div className="mt-0.5 p-2 rounded-lg bg-white border border-emerald-200 text-emerald-700">
                        <FileSignature className="w-4 h-4" />
                      </div>
                      <div>
                        <h4 className="text-xs font-black text-emerald-900 uppercase tracking-wider font-mono">
                          Data Privacy Consent Form
                        </h4>
                        <p className="text-[11px] leading-relaxed text-slate-700 mt-1">
                          I allow Barangay 35 Maypajo iLigtas to collect and process my name, contact details, account information, and emergency-related records for registration, disaster preparedness, alerting, rescue coordination, and barangay response services. I understand that my information will be protected and used only for official safety and response purposes.
                        </p>
                      </div>
                    </div>

                    <label className="flex items-start gap-2.5 text-[11px] leading-relaxed text-slate-800 font-semibold cursor-pointer">
                      <input
                        type="checkbox"
                        checked={dataConsentAccepted}
                        onChange={(e) => setDataConsentAccepted(e.target.checked)}
                        className="mt-0.5 h-4 w-4 rounded border-emerald-400 accent-emerald-600 cursor-pointer"
                        required
                      />
                      <span>I have read and agree to the Data Privacy Consent Form.</span>
                    </label>

                    <div>
                      <label className="block text-[10px] font-bold text-emerald-900 tracking-wider uppercase mb-1 font-mono">
                        E-Signature: Type Full Name *
                      </label>
                      <input
                        type="text"
                        required
                        value={dataConsentSignature}
                        onChange={(e) => setDataConsentSignature(e.target.value)}
                        placeholder="Must match the full name above"
                        className="w-full text-xs p-2.5 rounded border border-emerald-200 bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:border-emerald-500 font-mono"
                      />
                    </div>
                  </div>
